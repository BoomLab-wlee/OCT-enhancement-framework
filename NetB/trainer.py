...
# model_name = '210526_image_dispersion_test_ESRGAN_noise2noise_gaussian'
model_name = '20230104'
...
'''Dataset setting'''
path = '/projects/oct_enhancement/f_space/dataset'
trainloader = data_loader.get_loader(dataset_path=path, phase='train', shuffle=True, batch_size=16, num_workers=2)
valloader = data_loader.get_loader(dataset_path=path, phase='val', shuffle=True, batch_size=16, num_workers=2)
...
netD = nn.DataParallel(netD, device_ids=[0,1], output_device=0)
...
results = {'d_loss': [], 'g_loss': [], 'd_score': [], 'g_score': [], 'msssim': [], 'ssim': [],
           ...
           'd_loss_real': [], 'd_loss_fake': [], 'val_loss' : [], 'val_mse' : [], 'val_l1' : [],
           'val_ssim' : [], 'val_grad' : []}
...
for epoch in range(Epoch, NUM_EPOCHS + 1):
    ...
    generator_loss = {'MSE' : 0, 'L1' : 0, 'SSIM_loss' : 0, 'GRAD' : 0, 'Adversarial' : 0}
    ...
    with torch.autograd.set_detect_anomaly(True):
        for data, target in train_bar:
            ...
            g_loss = mse_loss * 0.6 + l1_loss * 0.8 + s_loss + grad_loss * 0.9
        ...
        if epoch % 100 == 0 or epoch == 1:
            result_show = torchvision.utils.make_grid(torch.cat((data[0:3, :], target[0:3, :], fake_img[0:3, :]), dim=0), nrow=3)
    ...
    with torch.no_grad():
        ...
        valing_results = {'MSE': 0, 'L1': 0, 'SSIM_loss': 0, 'GRAD': 0, 'loss' : 0, 'batch_sizes': 0}
        val_images = []
        for data, target  in val_bar:
            ...
            g_val_loss = mse_loss * 0.6 + l1_loss * 0.8 + s_loss + grad_loss * 0.9

            valing_results['MSE'] += (mse_loss*0.3).item() * batch_size
            valing_results['L1'] += (l1_loss*0.7).item() * batch_size
            valing_results['SSIM_loss'] += s_loss.mean().item() * batch_size
            valing_results['GRAD'] += (grad_loss*0.9).mean().item() * batch_size
            valing_results['loss'] += g_val_loss.data.mean().item() * batch_size

            val_bar.set_description(
                desc='[Validating results] Loss: %.4f MSE: %.4f L1: %.4f SSIM_loss: %.4f GRAD: %.4f' % (
                    valing_results['loss'] / valing_results['batch_sizes'],
                    valing_results['MSE'] / valing_results['batch_sizes'], valing_results['L1'] / valing_results['batch_sizes'],
                    valing_results['SSIM_loss'] / valing_results['batch_sizes'], valing_results['GRAD'] / valing_results['batch_sizes']))
    ...
    results['val_ssim'].append(valing_results['SSIM_loss'] / valing_results['batch_sizes'])
    ...
    writer.add_scalar('ValingResults/SSIM', valing_results['SSIM_loss'], epoch)
    ...
    if epoch >= 1:
        ...
        train_results = pd.DataFrame(
            data={'Loss_D': results['d_loss'], 'Loss_G': results['g_loss'], 'Score_D': results['d_score'],
                  ...
                  'val_loss' : results['val_loss'], 'val_mse' : results['val_mse'], 'val_l1' : results['val_l1'],
                  'val_ssim' : results['val_ssim'], 'val_grad' : results['val_grad']},
